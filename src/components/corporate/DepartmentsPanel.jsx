            ...
            return (
              <div key={d.id} className="bg-emerald-black ring-1 ring-white/10 rounded-lg p-5">
                ...
                <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
                  <div className="bg-forest-secondary/60 rounded-lg p-2">
      ...
      {leaderboard.length > 0 && (
        <div className="mt-8 bg-emerald-black ring-1 ring-white/10 rounded-lg p-6">
          ...
          <div className="space-y-2">
            {leaderboard.map((d, i) => (
              <div key={d.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-forest-secondary/60">
                ...
                <span className="text-sm font-bold text-[#D6B56D]">${d.sav.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <Modal title="Add department" onClose={() => setShowAdd(false)}>
          <form onSubmit={create} className="space-y-3">
            <Field label="Department name">
              <input required value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} placeholder="Marketing" className="w-full px-3 py-2 text-sm border border-white/12 rounded-lg outline-none focus:border-[#D6B56D]/40" />
            </Field>
            <Field label="Head email (optional)">
              <input type="email" value={form.head_email} onChange={(e) => setForm((s) => ({ ...s, head_email: e.target.value }))} placeholder="head@acme.com" className="w-full px-3 py-2 text-sm border border-white/12 rounded-lg outline-none focus:border-[#D6B56D]/40" />
            </Field>
            <Field label="Description (optional)">
              <textarea value={form.description} onChange={(e) => setForm((s) => ({ ...s, description: e.target.value }))} rows={2} className="w-full px-3 py-2 text-sm border border-white/12 rounded-lg outline-none focus:border-[#D6B56D]/40" />
            </Field>
            <div>
            ...
            <button type="submit" className="w-full bg-[#D6B56D] hover:bg-[#E5C77A] text-[#062B23] font-semibold py-2.5 rounded-lg text-sm">Save department</button>
          </form>
        </Modal>
      )}
    </div>
