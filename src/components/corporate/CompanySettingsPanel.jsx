      ...
      <form onSubmit={save}>
        {tab === "profile" && (
          <div className="bg-emerald-black ring-1 ring-[#F1F1F1] rounded-lg p-6 space-y-4 max-w-3xl">
            <h3 className="font-semibold text-ivory flex items-center gap-2"><Building2 className="w-4 h-4 text-[#1B4F9C]" /> Company profile</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex items-center">
                  ...
                  <input value={form.email_domain} onChange={(e) => update("email_domain", e.target.value.toLowerCase().replace(/^@/, ""))} className="flex-1 px-3 py-2 text-sm border border-[#F1F1F1] rounded-r-lg outline-none focus:border-[#1B4F9C]/40 focus:ring-2 focus:ring-[#F1F1F1]" />
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "branding" && (
          <div className="bg-emerald-black ring-1 ring-[#F1F1F1] rounded-lg p-6 space-y-4 max-w-3xl">
            <h3 className="font-semibold text-ivory flex items-center gap-2"><Palette className="w-4 h-4 text-[#1B4F9C]" /> HR portal branding</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="file" accept="image/*" onChange={(e) => onFile("logo_url", e)} className="block w-full text-xs text-ivory-muted file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-[#FFFFFF] file:text-[#1B4F9C] file:font-semibold file:text-xs mt-2" />
              </div>
              <div>
                ...
                <input type="file" accept="image/*" onChange={(e) => onFile("branding_logo_url", e)} className="block w-full text-xs text-ivory-muted file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-[#FFFFFF] file:text-[#1B4F9C] file:font-semibold file:text-xs mt-2" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea rows={3} value={form.branding_welcome_message} onChange={(e) => update("branding_welcome_message", e.target.value)} placeholder="Welcome to your company benefits portal!" className="w-full px-3 py-2 text-sm border border-[#F1F1F1] rounded-lg outline-none focus:border-[#1B4F9C]/40" />
            </div>
          </div>
        )}
        ...
        {tab === "sso" && (
          <div className="bg-emerald-black ring-1 ring-[#F1F1F1] rounded-lg p-6 space-y-4 max-w-3xl">
            <h3 className="font-semibold text-ivory flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-[#1B4F9C]" /> Single Sign-On</h3>
            <div>
              <label className="block text-xs font-semibold text-ivory-muted mb-1.5">SSO Provider</label>
              <select value={form.sso_provider} onChange={(e) => update("sso_provider", e.target.value)} className="w-full px-3 py-2 text-sm border border-[#F1F1F1] rounded-lg outline-none focus:border-[#1B4F9C]/40">
        ...
        {tab === "audit" && (
          <div className="bg-emerald-black ring-1 ring-[#F1F1F1] rounded-lg p-6 max-w-3xl">
            <h3 className="font-semibold text-ivory flex items-center gap-2 mb-3"><AuditIcon className="w-4 h-4 text-[#1B4F9C]" /> Recent changes by your team</h3>
            {audits.length === 0 ? (
...
function Input({ label, value, onChange, full }) {
  return (
    <div className={full ? "sm:col-span-2" : ""}>
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-2 text-sm border border-[#F1F1F1] rounded-lg outline-none focus:border-[#1B4F9C]/40 focus:ring-2 focus:ring-[#F1F1F1]" />
    </div>
  );
}