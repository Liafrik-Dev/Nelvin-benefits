      ...
      <div className="bg-emerald-black ring-1 ring-[#F1F1F1] rounded-lg overflow-hidden overflow-x-auto">
        {loading ? (
          ...
          <table className="w-full text-sm min-w-[760px]">
            <thead className="bg-[#F9F8F7] text-ivory-muted text-xs uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-[#F1F1F1]">
              {filtered.map((e) => (
                <tr key={e.id} className="hover:bg-[#F9F8F7]">
      ...
      {csvMode && (
        <Modal title="Upload employee CSV" onClose={() => setCsvMode(false)}>
          <div className="space-y-3">
            <div className="bg-[#FFFFFF] text-ivory rounded-lg p-3 text-xs flex gap-2">
...
function RowMenu({ onEdit, onSuspend, onActivate, onReset, onRemove }) {
  ...
  return (
    <div className="relative inline-block">
      <button onClick={() => setOpen((o) => !o)} className="p-1.5 rounded hover:bg-[#F4F4F4]"><MoreVertical className="w-4 h-4 text-ivory-muted" /></button>
      {open && (
        <>
...
function Modal({ title, children, onClose }) {
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FFFFFF] rounded-lg shadow-2xl border border-[#F1F1F1] w-full max-w-md p-6 max-h-[85vh] overflow-y-auto">
