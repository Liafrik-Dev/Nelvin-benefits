...
const inputClass =
  "w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-300 transition-all";
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1.5">Business Email</label>
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1.5">Business Category</label>
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1.5">Business Address <span className="text-gray-400 font-normal">(optional)</span></label>
          <input value={form.business_address} onChange={(e) => update("business_address", e.target.value)} className={inputClass} />
        </div>
      </div>

      <div>
        <label className="text-sm font-medium text-gray-700 block mb-1.5">Website or Facebook/Instagram Page <span className="text-gray-400 font-normal">(optional)</span></label>
        <input value={form.website} onChange={(e) => update("website", e.target.value)} className={inputClass} />
      </div>

      <div>
        <label className="text-sm font-medium text-gray-700 block mb-1.5">Brief description of your offer</label>
      ...
      <div>
        <label className="text-sm font-medium text-gray-700 block mb-1.5">
          Business Images <span className="text-gray-400 font-normal">(1920 × 1080px recommended)</span>
        </label>
        <label className="flex items-center gap-3 border border-dashed border-gray-300 rounded-2xl p-4 cursor-pointer hover:border-gray-900/40 hover:bg-gray-50 transition-all">
          ...
          <div className="w-10 h-10 rounded-xl bg-white border border-gray-100 flex items-center justify-center flex-shrink-0">
            {uploading ? <Loader2 className="w-4 h-4 animate-spin text-gray-400" /> : <ImagePlus className="w-4 h-4 text-gray-400" />}
          </div>
          <p className="text-sm text-gray-500">{uploading ? "Uploading..." : "Click to upload business photos"}</p>
        </label>
      ...
      <button
        ...
        onClick={onNext}
        className="w-full bg-gray-900 hover:bg-gray-800 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-full py-3.5 text-sm font-semibold transition-all hover:-translate-y-0.5 hover:shadow-lg"
      >
        Continue
      </button>
    </div>
