  ...
  return (
    <div className="space-y-6">
      ...
      <ul className="text-sm text-ivory-muted space-y-2 max-w-xs mx-auto">
        {["Look directly at the camera", "Good lighting", "No sunglasses or face covering"].map((rule) => (
          <li key={rule} className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-[#D6B56D] flex-shrink-0" />
            {rule}
          </li>
        ))}
      </ul>
      ...
      <div className="flex flex-col items-center gap-4">
        <div className="w-56 h-56 rounded-full bg-forest-secondary/60 border-2 border-dashed border-white/12 flex items-center justify-center overflow-hidden">
          {form.selfie_url ? (
        ...
        <label className="cursor-pointer bg-[#062B23] hover:bg-emerald-black text-white rounded-full px-6 py-3 text-sm font-semibold transition-all hover:-translate-y-0.5 hover:shadow-lg flex items-center gap-2">
