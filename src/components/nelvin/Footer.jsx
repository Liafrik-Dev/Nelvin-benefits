    ...
    <footer className="bg-[#1a0a0a] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8">
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="font-bold text-xs tracking-wider uppercase mb-4 text-white/80">{title}</h4>
              <ul className="space-y-2.5">
                {links.map(link => (
                  <li key={link}>
                    {linkRoutes[link] ? (
                      <Link to={linkRoutes[link]} className="text-white/50 hover:text-white text-xs transition-colors">{link}</Link>
                    ) : mailtoLinks[link] ? (
                      <a href={mailtoLinks[link]} className="text-white/50 hover:text-white text-xs transition-colors">{link}</a>
                    ) : (
                      <a href="#" className="text-white/50 hover:text-white text-xs transition-colors">{link}</a>
                    )}
                  </li>
                ))}
              </ul>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-white/30 text-xs">© 2026 Nelvin Africa Ltd. All rights reserved. Made across Africa.</p>
