...
export default function AdminDataTable({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      <div className="flex flex-col lg:flex-row lg:items-center gap-3 p-4 border-b border-gray-100">
        {setSearch && (
          <input
            value={search}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg whitespace-nowrap"
          >
            <Download className="w-3.5 h-3.5" /> Export CSV
          </button>
          <span className="text-xs text-gray-400 hidden lg:inline">{total} total</span>
        </div>
      </div>

      {setSelected && bulkActions.length > 0 && selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 px-4 py-2 bg-emerald-50 border-b border-emerald-100">
          <span className="text-sm text-emerald-700 font-medium">{selected.size} selected</span>
          {bulkActions.map((b, i) => (
            <button
              key={i}
              onClick={() => b.onClick(selected)}
              className="text-xs px-3 py-1.5 bg-white border bg-white rounded-md hover:bg-gray-50"
            >
              {b.label}
            </button>
          ))}
          <button onClick={() => setSelected(new Set())} className="text-xs text-gray-500 ml-auto">
            Clear
          </button>
        </div>
      )}
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 text-left text-xs uppercase text-gray-400 tracking-wide">
              {setSelected && (
            ...
            {loading ? (
              ...
              paged.map((row) => (
                ...
                >
                  ...
                  {columns.map((c) => (
                    <td key={c.key} className="px-3 py-2 text-sm text-gray-700 align-middle">
