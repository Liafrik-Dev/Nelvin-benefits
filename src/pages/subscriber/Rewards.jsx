      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-gradient-to-r from-black to-black rounded-xl p-6 sm:p-8 text-white shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="bg-[#F4F4F4] backdrop-blur border border-[#E3E3E3] p-6 rounded-lg text-center min-w-[200px]">
            <p className="text-xs uppercase tracking-wider text-[#1B4F9C] font-bold mb-1">Your Reward Balance</p>
            <p className="text-4xl font-black text-[#1B4F9C] font-heading">{points}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-emerald-black ring-1 ring-[#F1F1F1] rounded-xl p-6 shadow-sm space-y-4">
            <div className="flex items-center gap-2 text-[#1B4F9C] font-bold font-heading">
              <Send className="w-5 h-5 text-[#1B4F9C]" /> Send Peer Recognition
            </div>
            ...
            <form onSubmit={handleSendPraise} className="space-y-4 pt-2">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#F9F8F7] border border-[#F1F1F1] rounded-xl px-3.5 py-2.5 text-xs font-medium focus:bg-[#FFFFFF] focus:outline-none focus:ring-2 focus:ring-[#1B4F9C]/40"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#F9F8F7] border border-[#F1F1F1] rounded-xl p-3 text-xs font-medium focus:bg-[#FFFFFF] focus:outline-none focus:ring-2 focus:ring-[#1B4F9C]/40"
                />
              </div>
              ...
              {sentMsg && <p className="text-xs text-[#1B4F9C] font-bold text-center">Praise sent successfully!</p>}
            </form>
          </div>
          ...
          <div className="lg:col-span-2 bg-emerald-black ring-1 ring-[#F1F1F1] rounded-xl p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-[#F1F1F1] pb-3">
              <div className="flex items-center gap-2 font-bold text-ivory font-heading">
                <Sparkles className="w-5 h-5 text-[#1B4F9C]" /> Recognition Feed
              </div>
                ...
                <div key={item.id} className="p-4 rounded-lg bg-[#F9F8F7] border border-[#F1F1F1] space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold text-ivory">
                      <span className="text-[#1B4F9C]">{item.sender}</span> recognized <span className="text-[#1B4F9C]">{item.recipient}</span>
                    </p>
                    <span className="text-[10px] bg-[#F9F8F7] text-[#1B4F9C] font-bold px-2 py-0.5 rounded-full">
                      +{item.points} Pts
                    </span>
                  </div>
