  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Send system (in-app) and email notifications to specific audiences.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-4 py-2 text-sm rounded-lg border ${form.channel === "system" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-white border-gray-200 text-gray-600"}`}
              >
                System (in-app)
              </button>
              <button
                ...
                className={`px-4 py-2 text-sm rounded-lg border ${form.channel === "email" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-white border-gray-200 text-gray-600"}`}
              >
                Email
              </button>
            </div>
          ...
          <div>
            ...
            <select value={form.audience} onChange={(e) => set("audience", e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-100">
          ...
          {form.audience === "specific_users" && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Recipient Email</label>
              <input value={form.recipient_email} onChange={(e) => set("recipient_email", e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
            </div>
          )}
          {form.audience === "countries" && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Country</label>
              <input value={form.target_country} onChange={(e) => set("target_country", e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
            </div>
          )}
          {form.audience === "membership" && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Plan</label>
              <select value={form.target_membership_plan} onChange={(e) => set("target_membership_plan", e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white">
                <option value="">—</option>
          ...
          {form.audience === "companies" && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Company ID</label>
              <input value={form.target_company_id} onChange={(e) => set("target_company_id", e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
            </div>
          )}
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1.5">Title</label>
            <input value={form.title} onChange={(e) => set("title", e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1.5">Message</label>
            <textarea value={form.message} onChange={(e) => set("message", e.target.value)} rows={4} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
          </div>
          <button
            onClick={sendServer}
            disabled={sending}
          ...
          {sent > 0 && <p className="text-xs text-emerald-700">{sent} notifications delivered this session.</p>}
        </div>
            ...
            <ul className="divide-y divide-gray-100">
              {history.map((n) => (
                <li key={n.id} className="py-3">
                  ...
                  <p className="text-xs text-gray-400 mt-1">To: {n.audience} · {formatDate(n.created_date)}</p>
                </li>
              ))}
            </ul>
          )}
