...
export default function AdminPendingOffers() {
  ...
  const columns = [
    ...
    { key: "discount_label", label: "Discount", render: (o) => <span className="text-xs font-semibold text-[#1B4F9C]">{o.discount_label || "—"}</span> },
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold font-heading text-ivory">Pending Offers</h1>
        <p className="text-sm text-ivory-muted mt-1">{filtered.length} offers awaiting approval · Approve, reject or request changes.</p>
      </div>
      ...
      <AdminDataTable
        ...
        renderActions={(o) => (
          <div className="flex items-center justify-end gap-1">
            ...
            <RowIconBtn title="Approve & publish" Icon={Check} color="text-[#1B4F9C] hover:text-[#1B4F9C]" onClick={() => approve(o)} />
            <RowIconBtn title="Reject" Icon={X} color="text-rose-600 hover:text-rose-700" onClick={() => reject(o)} />
            <button title="Request changes" onClick={() => requestChanges(o)} className="px-2 py-1 text-xs bg-[#F9F8F7] text-[#1B4F9C] ring-1 ring-[#1B4F9C]/20 rounded hover:bg-[#F9F8F7]">Request changes</button>
          </div>
        )}
        bulkActions={[
        ...
        <div className="fixed inset-0 z-[200] flex justify-end">
          ...
          <div className="relative w-full max-w-lg bg-[#FFFFFF] h-full overflow-y-auto p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {viewing.review_notes && <div><span className="text-xs text-ivory-dim">Review Notes</span><p className="text-[#1B4F9C]">{viewing.review_notes}</p></div>}
            </div>
            <div className="mt-6 flex gap-2">
              <button onClick={() => approve(viewing)} className="flex-1 bg-[#1B4F9C] hover:bg-[#1B4F9C] text-white rounded-full py-2 text-sm font-semibold">Approve & publish</button>
