...
export default function BusinessCustomers() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold font-heading text-ivory">Audience & Customer Insights</h1>
        <p className="text-sm text-ivory-muted mt-1">Breakdown of top corporate employers sending members to your store.</p>
      </div>
        ...
        {companies.map((c) => (
          <div key={c.name} className="bg-white rounded-lg border border-white/10 p-6 space-y-3 shadow-sm">
            ...
            <div className="pt-2 border-t border-gray-50 text-xs space-y-1 text-ivory-muted">
              ...
              <p className="text-[#D6B56D] font-bold"><span className="font-bold">Total Sales:</span> {c.revenue}</p>
            </div>
          </div>
        ))}
      </div>
