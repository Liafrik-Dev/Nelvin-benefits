  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold font-heading text-gray-900">Departments</h1>
          <p className="text-sm text-gray-500 mt-1">Group your team and track per-department savings.</p>
        </div>
        <div className="flex gap-2">
          {PRESET.filter((p) => !items.some((i) => i.name === p)).slice(0, 3).map((p) => (
            <button key={p} onClick={() => { setForm((s) => ({ ...s, name: p })); setShowAdd(true); }}
              className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg">+ {p}</button>
          ))}
          <button onClick={() => setShowAdd(true)} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2 rounded-lg">
      ...
      {loading ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center text-sm text-gray-400">Loading…</div>
      ) : items.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          <Boxes className="w-10 h-10 text-gray-300 mx-auto mb-3" />
          <p className="text-sm text-gray-500">No departments yet. Create your first one above.</p>
        </div>
      ) : (
            ...
            return (
              <div key={d.id} className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex items-start justify-between">
                  ...
                  <button onClick={() => remove(d.id, d.name)} className="text-gray-300 hover:text-rose-600 p-1"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
                {d.description && <p className="text-xs text-gray-500 mt-1">{d.description}</p>}
                <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
                  <div className="bg-gray-50 rounded-lg p-2">
                    <p className="text-gray-400 uppercase tracking-wider">Employees</p>
                    <p className="text-sm font-bold text-gray-900">{emps.length}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-2">
                    <p className="text-gray-400 uppercase tracking-wider">Savings</p>
                    <p className="text-sm font-bold text-gray-900">${sav.toLocaleString()}</p>
                  </div>
                </div>
                {d.head_email && <p className="text-xs text-gray-400 mt-2">Head: {d.head_email}</p>}
              </div>
            );
          })}
        </div>
      ...
      {leaderboard.length > 0 && (
        <div className="mt-8 bg-white rounded-2xl border border-gray-100 p-6">
          <h3 className="font-semibold text-gray-900 mb-3">Department Leaderboard</h3>
          <div className="space-y-2">
            {leaderboard.map((d, i) => (
              <div key={d.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50">
                <span className="text-xs text-gray-400 w-5">{i + 1}</span>
                <span className="w-3 h-3 rounded-full" style={{ background: d.color }} />
                <span className="text-sm text-gray-700 flex-1">{d.name}</span>
                <span className="text-xs text-gray-500">{d.count} emp · </span>
                <span className="text-sm font-bold text-emerald-700">${d.sav.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <Modal title="Add department" onClose={() => setShowAdd(false)}>
          <form onSubmit={create} className="space-y-3">
            <Field label="Department name">
              <input required value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} placeholder="Marketing" className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-400" />
            </Field>
            <Field label="Head email (optional)">
              <input type="email" value={form.head_email} onChange={(e) => setForm((s) => ({ ...s, head_email: e.target.value }))} placeholder="head@acme.com" className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-400" />
            </Field>
            <Field label="Description (optional)">
              <textarea value={form.description} onChange={(e) => setForm((s) => ({ ...s, description: e.target.value }))} rows={2} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-400" />
            </Field>
            <div>
            ...
            <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2.5 rounded-lg text-sm">Save department</button>
          </form>
        </Modal>
      )}
    </div>
